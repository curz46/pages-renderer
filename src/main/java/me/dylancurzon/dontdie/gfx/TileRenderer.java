...
//    private VertexBuffer currentFrame;
    ...
    public void tilemapUpdate() {
        final Set<Vector2i> tiles = this.camera.getVisibleTiles();

        final float[] positions = new float[tiles.size() * 2 * 4];
        final float[] texCoords = new float[tiles.size() * 2 * 4];
        final int[] texIndex = new int[tiles.size() * 4];

        final Iterator<Vector2i> it = tiles.iterator();
        for (int i = 0; i < tiles.size(); i++) {
            final Vector2i coord = it.next();
//            int d = 0;
//            for (int xd = 0; xd <= 1; xd++) {
//                for (int yd = 0; yd <= 1; yd++) {
//                    positions[i * 2 * 4 + d++] = coord.getX() + xd;
//                    positions[i * 2 * 4 + d++] = coord.getY() + yd;
//                }
//            }#
            positions[i * 2 * 4 + 0] = coord.getX();
            positions[i * 2 * 4 + 1] = coord.getY();
            positions[i * 2 * 4 + 2] = coord.getX() + 1;
            positions[i * 2 * 4 + 3] = coord.getY();
            positions[i * 2 * 4 + 4] = coord.getX() + 1;
            positions[i * 2 * 4 + 5] = coord.getY() + 1;
            positions[i * 2 * 4 + 6] = coord.getX();
            positions[i * 2 * 4 + 7] = coord.getY() + 1;

//            final TileType type = level.getTile(coord)
//                .orElseThrow(() -> new RuntimeException("No tile type!"));
            final TileType type = TileType.STONEBRICKS;
...
//        glEnableVertexAttribArray(5);
        glUniform2fv(0, new float[] { (float) this.cameraFixed.getX(), (float) this.cameraFixed.getY() });
        glUniform2fv(1, new float[] { (float) this.cameraSize.getX(), (float) this.cameraSize.getY() });
        glUniform2fv(2, new float[] { (float) this.cameraDelta.getX(), (float) this.cameraDelta.getY() });
